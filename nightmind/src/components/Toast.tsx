...
export function ToastHost() {
  ...
  return (
    <div
      aria-live="polite"
      className="pointer-events-none fixed inset-x-0 bottom-[104px] z-50 flex justify-center px-5 md:bottom-8 md:pl-[88px] xl:pl-[240px]"
    >
      <AnimatePresence>
        {message ? (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            transition={m.t(180)}
            className="rounded-full bg-raised px-5 py-3 t-label text-ink card-shadow"
          >
            {message}
          </motion.div>
        ) : null}
